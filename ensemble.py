...
#Glove Embedding ( semantic embedding )
...
def getSentimentEmbeddingMatrix(wordIndex):
    ...
    embeddingsIndex = {}
    # Load the embedding vectors from ther GloVe file
    with io.open(os.path.join("SSWE", 'sswe-u.txt'), encoding="utf8") as f:
        for line in f:
            values = line.split()
            word = values[0]
            embeddingVector = np.asarray(values[1:], dtype='float32')
...
def buildModel(embeddingMatrix,sentimentEmbeddingMatrix):  
    """
    embeddingLayer1 = Embedding(embeddingMatrix.shape[0],
                            EMBEDDING_DIM,
                            weights=[embeddingMatrix],
                            input_length=MAX_SEQUENCE_LENGTH,
                            trainable=False)
    model1 = Sequential()
    model1.add(embeddingLayer1)
    model1.add(LSTM(LSTM_DIM, dropout=DROPOUT, activation="relu"))

    #LSTM with Sentiment embedding
    embeddingLayer2 = Embedding(sentimentEmbeddingMatrix.shape[0],
                            50,
                            weights=[sentimentEmbeddingMatrix],
                            input_length=MAX_SEQUENCE_LENGTH,
                            trainable=False)
    model2 = Sequential()
    model2.add(embeddingLayer2)
    model2.add(LSTM(LSTM_DIM, dropout=DROPOUT, activation="relu"))
    
    commonInput = Input((100,))
    ...
    model = Model(commonInput, mergedOut)
    ...
    embeddingLayer1 = Embedding(embeddingMatrix.shape[0],
                            ...
                            weights=[embeddingMatrix],
                            input_length=MAX_SEQUENCE_LENGTH,
                            trainable=False)(inputTensor)

    semanticLSTM = LSTM(128, dropout=DROPOUT)(embeddingLayer1)
    act_1 = Activation('relu')(semanticLSTM)

    dense_1 = Dense(NUM_CLASSES, activation='sigmoid')(act_1)

    model_1 = Model(inputTensor, dense_1)   
    rmsprop = optimizers.rmsprop(lr=LEARNING_RATE)
    model_1.compile(loss='categorical_crossentropy',
                  optimizer=rmsprop,
                  metrics=['acc'])

    #load saved model, for semantic LSTM
    model_1 = load_model('glove_model.h5')
    ...
    sentiLSTM = LSTM(128, dropout=DROPOUT)(embeddingLayer2)
    act_2 = Activation('relu')(sentiLSTM)

    dense_2 = Dense(NUM_CLASSES, activation='sigmoid')(act_2)
    model_2 = Model(inputTensor, dense_2)       
    model_2.compile(loss='categorical_crossentropy',
                  optimizer=rmsprop,
                  metrics=['acc'])
    
    model_2 = load_model('sentiment_model.h6')

    mergedOut = Concatenate()([act_1,act_2])
    act_3 = Activation('relu')(mergedOut)
    dense = Dense(128, activation='relu')(act_3)
    denseDrop = Dropout(DROPOUT)(dense)

    dense64 = Dense(64, activation='relu')(dense)
    denseDrop64 = Dropout(DROPOUT)(denseDrop)
    
    output = Dense(NUM_CLASSES, activation='softmax')(denseDrop64)
...
def main():
    ...
    sentimentEmbeddingMatrix = getSentimentEmbeddingMatrix(wordIndex)
    data = pad_sequences(trainSequences, maxlen=MAX_SEQUENCE_LENGTH)
    labels = to_categorical(np.asarray(labels))

    testlabelsCat = to_categorical(np.asarray(testlabels))
    ...
    model = buildModel(embeddingMatrix,sentimentEmbeddingMatrix)
    model.fit(data, labels, epochs=NUM_EPOCHS, batch_size=BATCH_SIZE)
    model.save('EP%d_LR%de-5_LDim%d_BS%d.h21'%(NUM_EPOCHS, int(LEARNING_RATE*(10**5)), LSTM_DIM, BATCH_SIZE))

    model = load_model('EP%d_LR%de-5_LDim%d_BS%d.h21'%(NUM_EPOCHS, int(LEARNING_RATE*(10**5)), LSTM_DIM, BATCH_SIZE))
    ...
    accuracy, microPrecision, microRecall, microF1 = getMetrics(predictions, testlabelsCat)
    predictions = predictions.argmax(axis=1)

    print(confusion_matrix(testlabels, predictions, labels=[0,1,2,3]))
